...
const svgTshirt = ref<HTMLIFrameElement | null>(null);
...
const loadReactivity = async () => {
  svgTshirt.value = document.getElementById("svgTshirt") as HTMLIFrameElement;
  if (svgTshirt.value) {
    svgTshirt.value.style.width = "100%";

    svgTshirt.value.style.height =
      svgTshirt.value.getBoundingClientRect().width * 1.25 + "px";
    while (!svgTshirtFill.value) {
      if (svgTshirt.value.contentWindow) {
        svgTshirtFill.value =
          svgTshirt.value.contentWindow.document.getElementById("tshirtFill");
      }
      await new Promise((r) => setTimeout(r, 200));
    }
    if (
      tshirtContainer.value &&
      selectedImage.value &&
      selectedImageContainer.value
    ) {
      tshirtContainer.value.style.height =
        svgTshirt.value?.getBoundingClientRect().height < window.innerHeight
          ? svgTshirt.value?.getBoundingClientRect().height + "px"
      ...
      selectedImage.value.style.height =
        svgTshirt.value?.getBoundingClientRect().height * 0.3 + "px";
      selectedImageContainer.value.style.height =
        selectedImage.value.getBoundingClientRect().height * 1.5 + "px";
      selectedImageContainer.value.style.width =
        selectedImage.value.getBoundingClientRect().width * 1.5 + "px";
    }
  }

  let colorCircles = document.querySelectorAll(
    "div#leftSelect div, div#rightSelect div"
  ) as NodeListOf<HTMLDivElement>;
  while (!colorCircles) {
    colorCircles = document.querySelectorAll(
      "div#leftSelect div, div#rightSelect div"
    );
  }
  if (colorCircles) {
...
onMounted(async () => {
  ...
  selectedImage.value?.addEventListener("mousedown", (e) => {
    if (selectedImage.value) {
      ...
      if (selectedImage.value) {
        selectedImage.value.style.marginTop =
          selectedImage.value.style.marginTop || "0";
        selectedImage.value.style.marginLeft =
      ...
      if (Boundary) {
        ...
        const handleMouseMove = (moveEvent: MouseEvent) => {
          if (Boundary && selectedImage.value) {
            const margin: number[] = [
              initMarginTop + moveEvent.screenY - initMousePosY,
              initMarginLeft + moveEvent.screenX - initMousePosX,
            ];

            if (scaler.value) {
              selectedImage.value.style.marginTop =
                scaler.value.style.marginTop =
                  maxMarginTop < margin[0] && margin[0] < maxMarginBottom
                    ? `${margin[0]}px`
              ...
              selectedImage.value.style.marginLeft =
                scaler.value.style.marginLeft =
                  maxMarginLeft < margin[1] && margin[1] < maxMarginRight
                    ? `${margin[1]}px`
  ...
  scaler.value?.addEventListener("mousedown", (mouseEvent: MouseEvent) => {
    if (selectedImage.value && scaler.value) {
      ...
      const HandleMouseMove = (mouseEvent: MouseEvent) => {
        if (selectedImage.value && scaler.value) {
          scaler.value.style.marginTop =
            (marginTop + mouseEvent.screenY - initMousePosY) * 0.9 + "px";
          selectedImage.value.style.height =
            initImgSize + mouseEvent.screenY - initMousePosY + "px";
        }
      };
...
<template>
  <div>
    ...
    <div id="TShirtExamples" class="loop_3">
      <LoopLeftRight
        :imgs="tShirtList"
        :url-prefix="urlPrefixTShirt"
        :img-mix="digitalArtList"
...
.scaler {
  ...
  background: radial-gradient(
    circle,
    rgba(153, 153, 255, 1) 0%,
    rgba(153, 153, 255, 1) 15%,
    rgba(0, 0, 0, 0) 16%,
    rgba(0, 0, 0, 0) 100%
  );
  border-radius: 50%;
