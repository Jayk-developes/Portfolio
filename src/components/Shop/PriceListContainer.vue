...
const addToCart = (price: string, object: string) => {
  store.addToCart({
    product: object + "-" + props.title,
    cost: parseInt(price.toString()),
    amount: 1,
  });
};
...
onMounted(() => {
  ...
  if (priceList.value) {
    priceList.value.style.top =
      -priceList.value.getBoundingClientRect().height + "px";
    priceList.value.style.fontSize =
      window.innerWidth / 400 >= 1 ? "1rem" : window.innerWidth / 400 + "rem";

    if ((window.innerWidth < 800 && window.innerWidth < window.innerHeight) || mobileAndTabletCheck() ) {
...
const expandPrice = () => {
  if ((window.innerWidth < 800 || mobileAndTabletCheck()) && priceList.value) {
    priceList.value.style.width =
      parseInt(priceList.value.style.width) > 2 ? "1.5rem" : "15rem";
  }
};
...
<template>
  <div class="digitalArtPriceList" ref="priceList" @click.stop="expandPrice">
    ...
    <div v-for="(key, value) in props.priceList" class="priceContainer">
      ...
      <div>
        <img
          src="../../media/shop/ShoppingIconSmall.png"
          alt="shoppingCart"
          style="width: auto; height: 1.5rem"
          @click="addToCart(key, value.toString())"
        />
      </div>
    </div>
  </div>
