...
const isElementAbove = (
  ...
  if (bottomElement && topElement) {
    bottomElement.style.top =
      topElement.offsetTop + topElement.offsetHeight + pixelAmount + "px";
  }
};
  ...
  navbar_items.forEach((item) => {
    item.addEventListener("click", () => {
      if (item.getAttribute("data-target") == "content_home") {
        ...
        if (targetID) {
          let scroller = document.getElementById(targetID) as HTMLElement;
...
<template>
  ...
  <div class="BG" id="BG2" ref="BG2"></div>
